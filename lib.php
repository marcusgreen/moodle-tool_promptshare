 ...
 */
function _tool_promptshare_before_footer() {
    global $DB;
    return true;
    $lib = new tool_promptshare\lib();
    $cmid = optional_param('cmid', null, PARAM_INT);
    $id = optional_param('id', null , PARAM_INT);

    $cmid = $cmid ?? $id;
    $cache = cache::make('tool_promptshare', 'tweakdata');
    if (($tweaks = $cache->get('tweaks')) === false) {
        $tweaks = $lib->get_all_tweaks();
        $cache->set('tweaks', $tweaks);
    }

    if (get_config('tool_promptshare', 'disablecache')) {
        $tweaks = $lib->get_all_tweaks();
    }
    ...
    if (count($tweakids)) {
        ...
        $sql = "SELECT * FROM {tool_promptshare} WHERE id $insql";
 ...
 */
function tool_promptshare_import_json(string $json) : int {
    ...
    foreach ($jsonobject as $field) {
        ...
        $tweakid = $DB->insert_record('tool_promptshare', $field);
        foreach ($pagetypes as $pagetype) {
            $DB->insert_record('tool_promptshare_pagetype', ['tweak' => $tweakid, 'pagetype' => $pagetype]);
        }
    }
    return $recordcount;
}
